...
class LevelSelectMode(Mode):
    def appStarted(mode):
        ...
        mode.gameModeNames = ["single player", "multiplayer",\
            "multiplayer + planets"]
        mode.gameModes = [mode.app.singlePlayerMode, mode.app.gameMode,\
    ...
    def getCell(mode, x, y):
        # returns cell and grid that it belongs to
        gridType = LevelSelectMode.pointInGrid(mode, x, y)
        if gridType == None:
            return (-1, -1)
        elif gridType == "levels":
    ...
    def getCellBounds(mode, col, height, numCells):
        ...
        #y0 = mode.width//2 - mode.cellHeight//2
        #y1 = mode.width//2 + mode.cellHeight//2
